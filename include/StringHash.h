...
namespace utl {
	...
	struct StringHash64
	{
		...
		constexpr StringHash64(const std::string_view str) : hash(details::Fnv1A64(str)) {}

		template <size_t N>
		constexpr StringHash64(const char(&str)[N])
			: hash(details::Fnv1A64(std::string_view(str, N - 1))) {
		}
	...
	{
		uint32_t hash;
		constexpr StringHash32(const std::string_view str) : hash(details::Fnv1A32(str)) {}
	...
	inline namespace Literals {
		...
		constexpr StringHash operator""_hs(const char* str, size_t len) noexcept { return { std::string_view(str, len) }; }
	}
