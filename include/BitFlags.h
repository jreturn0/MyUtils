...
namespace utl {
    ...
    public:
        ...
        // Bitwise operators
        constexpr BitFlags operator&(BitFlags const& rhs) const noexcept { return  BitFlags(m_mask & rhs.m_mask); } // AND
        constexpr BitFlags operator|(BitFlags const& rhs) const noexcept { return  BitFlags(m_mask | rhs.m_mask); } // OR
        constexpr BitFlags operator^(BitFlags const& rhs) const noexcept { return  BitFlags(m_mask ^ rhs.m_mask); } // XOR
        constexpr BitFlags operator~() const noexcept { return BitFlags(m_mask ^ static_cast<MaskType>(-1)); } // NOT
        ...
        constexpr void all() noexcept { m_mask = ~MaskType{ 0 }; }

        //chainable setters
        constexpr BitFlags& with(EnumType bit) noexcept { set(bit); return *this; }
        constexpr BitFlags& without(EnumType bit) noexcept { clear(bit); return *this; }
